...
**Current status**: V2 reboot. Phases 0-7f are complete. Phase 8 tasks 1-3 (xterm.js browser terminal over `/ws/terminal`) are implemented. Phase 8 tasks 4-6 (desktop chrome, GUI apps) and Phase 9 (game-reactive systems) are the next work. The legacy v1 code lives on the `legacy/v1` branch and should never be merged.

**Test counts (as of the latest collect)**:

- Backend pytest suite: **2,477 tests** collected.
...
- **Phase 9**: game-reactive systems (flags/quest state, NPC perception filters, knowledge gates, NPC initiative, Director/Fate system, Act 1 vertical slice). See `docs/PHASE_9_PLAN.md`.
