...
class TestFilesystemSecurity:
    ...
    def test_copy_operation_stays_within_virtual_filesystem(self, app_service):
        ...
        dst_dir = app_service.create_directory(
            {"name": "destination", "parent_id": root.id}
        )
...
def test_no_real_filesystem_access():
    ...
    # Get all public methods of AppService
    methods = {m for m in dir(AppService) if not m.startswith("_")}

    # The only methods that should interact with the real file system:
    allowed_fs_methods = {
    ...
    # Verify these expected methods actually exist
